...
class Plugin:
	...
	def trigger_word(self, msg):
		...
		# there's a game underway, check the word
		else:
	...
	def word_help(self, msg):
		...
		if len(msg.args) == 0:
			...
			try:
				self.c.notice(msg.nick, getattr(self, 'word_'+msg.args[0]).__doc__)
			except AttributeError:
				self.c.notice(msg.nick, "That command does not exist. For a list of commands, run %sword help."%self.c.command_prefix)
